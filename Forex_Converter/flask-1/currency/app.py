...
# f45dbac85ffd95fe522eba33a0802b81
...
def handle_form():
    ...
    access_key = "7021dff996bb4f2d060f79bf"
    url = f"https://v6.exchangerate-api.com/v6/{access_key}/pair/{code_from}/{code_to}/{amt}"
    
    response = requests.get(url)
    data = response.json()
    results = float(data["conversion_result"])
    ...
    if not errs:
        result = currency.convert_to_pretty(results, code_to)
        if result is None:
